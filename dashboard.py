...
year_options = ['Seluruhnya'] + [str(y) for y in years]
...
# Filter Data Logic
if selected_year_opt == 'Seluruhnya':
    df_filtered = df.copy()
...
if len(df_filtered) > 0:
    # Untuk "Seluruhnya", ambil rata-rata RR per daerah lalu cari max
    if selected_year_opt == 'Seluruhnya':
...
with row2_col1:
    with st.container(border=True):
        ...
        if len(gdf_filtered) > 0:
            # Jika Seluruhnya, rata-ratakan RR & PPM per daerah
            if selected_year_opt == 'Seluruhnya':
